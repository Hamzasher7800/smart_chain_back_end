    ...
    return {
        ...
        createOrder: async (req, res) => {
            try {
                ...
                const receipt = await contract.methods.createOrder(coordinates, coordinates).send({
                    from: fromAddress,
                    value,
                ...
                const nearbyProviders = await getNearbyServiceProviders(latitude, longitude);
        ...
        getAvailableOrdersByLocation: async (req, res) => {
            try {
                ...
                const allOrders = await contract.methods.getAvailableOrders().call();
                const nearbyOrders = allOrders.filter((order) => {
                    const [orderLat, orderLong] = order.userCoordinates.split(',').map(Number);
                    const distance = geolocation.calculateDistance(
                        { latitude: orderLat, longitude: orderLong },
